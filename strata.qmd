...
The top panel shows the population split into its strata, each drawn as a
...
## for either stratification variable ("acres82" quartiles or "region")
make_strata <- function(method) {
  ...
  y_pop <- (H + 1 - band) + runif(N, -0.30, 0.30)   # stratum 1 in the top band

  list(lev = levels(stratum), H = H, idx_h = idx_h,
       Nh = Nh, Sh = Sh, W = W, y_pop = y_pop)
}
...
server <- function(input, output, session) {
  ...
  output$popPlot <- renderPlot({
    s  <- si()
    H  <- s$H; Nh <- s$Nh; lev <- s$lev; y_pop <- s$y_pop
