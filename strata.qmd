...
Where $W_h = \frac{N_h}{N}$ is the stratum weight, and $\bar{y}_h$ is the sample mean of stratum $h$.

**2. Variance of the Stratified Mean**
The estimated variance accounts for the finite population correction in each stratum:
$$ \hat{V}(\bar{y}_{str}) = \sum_{h=1}^H W_h^2 \left(1 - \frac{n_h}{N_h}\right) \frac{s_h^2}{n_h} $$
...
format_str_gt <- function(est_vec, is_total = FALSE, is_prop = FALSE) {
  ...
  if (is_total) {
    est_sym <- "$\\hat{t}_{str}$"
    se_sym <- "$SE(\\hat{t}_{str})$"
  } else if (is_prop) {
    est_sym <- "$\\hat{p}_{str}$"
    se_sym <- "$SE(\\hat{p}_{str})$"
  } else {
    est_sym <- "$\\bar{y}_{str}$"
    se_sym <- "$SE(\\bar{y}_{str})$"
  }
...
We first manually compute the sample size ($n_h$), standard deviation ($s_h$), and mean ($\bar{y}_h$) for each region, matching them with the known total counties in each region ($N_h$).
...
data.frame(Ch, Nh, Sh, NhShDCh, Lh) |> 
  gt(rownames_to_stub = TRUE) |> 
  cols_label(NhShDCh = md("$N_h S_h / \\sqrt{C_h}$"), Lh = md("$L_h$ (Prop)")) |>
