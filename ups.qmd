...
This app estimates the population mean of `acres92` in the `agpop` data in
three ways, using the same sample size $n$. It uses two auxiliary variables, and you
can pick each one in the app:

* $M_i$, the **size variable for UPS**, which sets the selection probabilities
  $\psi_i = M_i / M_0$, where $M_0 = \sum_{k=1}^N M_k$;
* $x_i$, the **auxiliary variable for estimation**, used by the SRS ratio estimator,
  with known population mean $\bar{x}_U$.

The three estimators are:

* **SRS + average**: the sample mean $\bar{y}$ of a simple random sample drawn without replacement;
* **SRS + ratio**: the ratio estimator $\hat{B}\bar{x}_U$ from the same SRS, with $\hat{B} = \sum y_i / \sum x_i$;
* **UPSWR + HH**: the Hansen–Hurwitz estimator $\frac{1}{N}\cdot\frac{1}{n}\sum_{i=1}^n y_i/\psi_i$ from a separate sample drawn with replacement. The SE uses the $n$ values $y_i/(N\psi_i)$.

The left panel shows the population as a scatterplot of $y$. The SRS view plots $y$
...
Because $\psi_i = M_i/M_0$, the HH estimate of the total is
$\hat{t}_{HH} = M_0 \cdot \frac{1}{n}\sum y_i/M_i$. So HH behaves like a line through
the origin in the $(M, y)$ plane, with slope equal to the average of $y_i/M_i$.
The SRS ratio estimator instead fits a line in the $(x, y)$ plane with slope
$\hat{B} = \sum y_i/\sum x_i$. Each view draws its own fitted line, along with the
true line through the origin (slope $t_y/M_0$ or $t_y/t_x$).

The right panel collects the estimates from repeated samples as violins. It also shows
the current 95% intervals (red when they miss $\bar{y}_U$), the running coverage rates,
...
server <- function(input, output, session) {
  ...
  output$mainplot <- renderPlot({
    ...
    leg <- rbind(leg, data.frame(l = if (ups) "true y = (ty/M0) M" else "true y = (ty/tx) x",
                                 c = col_true, lt = 1, p = NA, w = 1.8))
    if (ups && "hh" %in% sel) leg <- rbind(leg, data.frame(
      l = "HH slope mean(y/M)", c = COL[["hh"]], lt = 2, p = NA, w = 2))
    ...
    plot(NA, xlim = c(0.4, k_n + 0.6), ylim = c(lo, hi), axes = FALSE,
         xlab = "", ylab = paste0("estimate of mean acres92", unit_lab(divy)),
