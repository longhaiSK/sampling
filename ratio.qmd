...
*   **Estimated Variance:** $\hat{V}(\overline{y}_r) = \left(\frac{\overline{x}_{\text{U}}}{\overline{x}}\right)^2 \left(1 - \frac{n}{N}\right) \frac{s_e^2}{n}$, built from the residuals
    $$
    e_i = y_i - \hat{B}x_i, \qquad s_e^2 = \frac{1}{n-1}\sum_{i=1}^n e_i^2 .
    $$
    $e_i$ is the vertical distance from the point $(x_i, y_i)$ to the fitted line through
    the origin, $y = \hat{B}x$. No centring appears in $s_e^2$ because
    $\hat{B} = \overline{y}/\overline{x}$ forces
    $\sum_i e_i = \sum_i y_i - \hat{B}\sum_i x_i = 0$ exactly, so the residuals already
    average to zero; the divisor is $n-1$ for the one parameter $\hat{B}$ estimated from
    the sample. Compare this with the SRS variance $\left(1 - \frac{n}{N}\right)s_y^2/n$:
    ratio estimation replaces the scatter of $y$ about $\overline{y}$ by the scatter about
    the line. That is the whole source of the gain --- and the whole risk, since a $x$ that
    predicts $y$ poorly gives $s_e^2 > s_y^2$ and a *less* precise estimate than the plain
    sample mean.
...
*   **Estimated Variance:** $\hat{V}(\overline{y}_{reg}) = \left(1 - \frac{n}{N}\right) \frac{s_e^2}{n}$, with
    $$
    e_i = y_i - \hat{B}_0 - \hat{B}_1 x_i, \qquad s_e^2 = \frac{1}{n-2}\sum_{i=1}^n e_i^2 ,
    $$
    the ordinary least-squares residuals and their mean square error. The divisor is now
    $n-2$, since both $\hat{B}_0$ and $\hat{B}_1$ are estimated. For the same data
    $\sum_i e_i^2$ can only be smaller here than under the ratio fit --- the ratio line is
    the special case $\hat{B}_0 = 0$ --- so the regression estimator is the safer of the
    two whenever the relationship misses the origin, at the cost of one degree of freedom.
    Note also that the factor $(\overline{x}_{\text{U}}/\overline{x})^2$ is absent: the
    regression estimator is evaluated at $\overline{x}_{\text{U}}$ directly, so no
    correction for $\overline{x} \neq \overline{x}_{\text{U}}$ is needed.
...
If we know the true population sizes ($N_h$) of strata, but we used a Simple Random Sample, we can post-stratify to reduce variance. Write $W_h = N_h/N$ for the known population weights, and let $n_h$, $\overline{y}_h$ and $s_h^2$ be the count, mean and variance of the units the sample happened to place in stratum $h$ (so $\sum_h n_h = n$, and each $n_h$ is random).

*   **Mean Estimator:** $\overline{y}_{post} = \sum_h W_h \overline{y}_h$ --- the stratified estimator, but with the *known* population weights in place of the sample proportions $n_h/n$.
*   **Expected sample sizes:** the variance is computed as if the design had produced the proportional allocation
    $$
    n_h^{post} = n\,W_h = n\frac{N_h}{N} .
    $$
*   **Estimated Variance:** substituting $n_h^{post}$ into the stratified variance, and using $n_h^{post}/N_h = n/N$ for every $h$,
    $$
    \hat{V}(\overline{y}_{post})
    = \sum_h \left(1 - \frac{n_h^{post}}{N_h}\right) W_h^2 \frac{s_h^2}{n_h^{post}}
    = \left(1 - \frac{n}{N}\right)\frac{1}{n}\sum_h W_h s_h^2 .
    $$
    This is exactly the variance of a proportionally allocated stratified sample: knowing the $N_h$ after the fact recovers almost all of the precision that stratifying in advance would have given. The approximation is treating the $n_h$ as fixed at $n W_h$; carrying the randomness of $n_h$ adds a second-order term $\frac{1}{n^2}\sum_h (1 - W_h)s_h^2$, which is negligible unless some $n_h$ are small. Every stratum needs $n_h \geq 2$ for $s_h^2$ to exist at all.



### Post-stratification Viewed as a Special Regression Estimation {-}

Post-stratification is not a separate technique: it is the regression estimator of §2
with the **stratum indicators** as the auxiliary variables. Take the $H$ dummies
$x_{ih} = \mathbf{1}\{i \in \text{stratum } h\}$ and fit without an intercept. Then

*   the fitted coefficients are the stratum means, $\hat{B}_h = \overline{y}_h$;
*   the population means of those dummies are the known weights, $\overline{x}_{\text{U},h} = N_h/N = W_h$;

so the regression estimator *is* the post-stratified one,
$$
\overline{y}_{reg} = \sum_h \hat{B}_h\,\overline{x}_{\text{U},h} = \sum_h W_h \overline{y}_h = \overline{y}_{post} .
$$

Its residuals are each unit's deviation from its own stratum mean,
$e_i = y_i - \overline{y}_{h(i)}$, and the general variance $\left(1-\frac{n}{N}\right)s_e^2/n$
applies with $p = H$ fitted parameters:
$$
...
\underbrace{W_h = \frac{N_h}{N}}_{\text{population proportions}} ,
$$
so the substitution $n_h \to n W_h$ of the previous subsection is simply: *pool the
residuals with the weights we know, not with the ones the sample happened to deliver.*
It is the same reasoning that puts $\overline{x}_{\text{U}}$, rather than $\overline{x}$,
in the ratio estimator.

Seen this way, the estimators above are one estimator with different auxiliary variables,
and $s_e^2 = \frac{1}{n-p}\sum_i e_i^2$ throughout, where $p$ is the number of fitted
parameters:
...
| Post-stratification | $H$ stratum indicators | $y_i - \overline{y}_h$ | $H$ |

Domain estimation stands slightly apart: there the divisor is the random $n_d$ rather than
the full $n$, because only the units falling in the domain carry information about it.
