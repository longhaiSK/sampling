...
server <- function(input, output, session) {
  ...
  observeEvent(list(input$n, input$xvar, input$logx), {
    rv$ests <- NULL; draw_sample()
  }, ignoreInit = FALSE)
  ...
  output$mainplot <- renderPlot({
    ...
    points(tfx(a$x[s]), tfy(ypl[s]), pch = 16, cex = 1.2, col = col_samp)
    ...
    leg <- data.frame(l = "sampled y", c = col_samp, lt = NA, p = 16, w = NA)
    ...
    legend("topleft", bty = "n", cex = 0.75, inset = c(0.15, 0.01),
           legend = leg$l, col = leg$c, lty = leg$lt, pch = leg$p, lwd = leg$w)

    ## ---- right: sampling distributions, on their own narrow scale ----
    par(mar = c(4.2, 4.8, 3.0, 1.0))
    br <- seq(lo, hi, length.out = 41)
    yy <- seq(lo, hi, length.out = 300)
    dens <- lapply(sel, function(k) dnorm(yy, ybarU, se_th[[k]]))
    names(dens) <- sel
    hs <- NULL
    if (nrep > 0) {
      hs <- lapply(sel, function(k)
        hist(pmin(pmax(M[, paste0("est_", k)], lo), hi), breaks = br, plot = FALSE)$density)
      names(hs) <- sel
    }
    dmax <- max(unlist(dens), unlist(hs))

    plot(NA, xlim = c(0, 1.05), ylim = c(lo, hi), axes = FALSE,
         xlab = "scaled density", ylab = paste0("estimate", unit_lab(divy)),
         main = "Sampling distributions of the estimates")
    rect(0, lo, 1.05, hi, col = col_band, border = NA)
    ord <- sel[order(se_th[sel], decreasing = TRUE)]   # widest at the back
    if (!is.null(hs)) for (k in ord)
      rect(0, br[-41], hs[[k]] / dmax, br[-1],
           col = adjustcolor(COL[[k]], alpha.f = 0.40), border = NA)
    for (k in ord) lines(dens[[k]] / dmax, yy, col = COL[[k]], lwd = 2)
    ...
    cmp <- setdiff(sel, "srs")
    if (length(cmp)) {
      red_txt <- vapply(cmp, function(k) {
        mc <- if (nrep > 5) 1 - var(M[, paste0("est_", k)]) / var(M[, "est_srs"]) else NA
        sprintf("%s %s (theory %s)", LAB[[k]], pct(mc), pct(1 - a$V[[k]] / Vy))
      }, "")
      legend("bottomleft", bty = "n", cex = 0.75, inset = c(0.15, 0.01),
             title = "Variance reduction vs. average", fill = COL[cmp], border = NA,
             legend = red_txt)
    }
  })
}
