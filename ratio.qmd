...
ybarU <- mean(pop$y)
xbarU <- mean(pop$x)
B     <- sum(pop$y) / sum(pop$x)     # population ratio, for the true line
...
cut_at_centre <- function(rmin, rmax, m) {
  ...
  if (!is.finite(cut) || cut >= rmax) as.numeric(quantile(c(rmin, rmax), 0.95)) else cut
}
cuty <- cut_at_centre(rgy[1], rgy[2], ybarU)
cutx <- cut_at_centre(rgx[1], rgx[2], xbarU)
...
ci_bar <- function(x, est, ci, col, pch, tf = identity, cap = 0.012) {
  segments(x, tf(ci[1]), x, tf(ci[2]), col = col, lwd = 2)
  segments(x - cap, tf(ci), x + cap, tf(ci), col = col, lwd = 2)
...
server <- function(input, output, session) {
  ...
  draw_sample <- function() {
    ...
    xs   <- pop$x[s]
    Bhat <- sum(ys) / sum(xs)
    est_r <- (sum(ys) + Bhat * sum(pop$x[-s])) / N
    est_a <- mean(ys)
    fpc   <- 1 - n / N
    se_r  <- (xbarU / mean(xs)) * sqrt(fpc * var(ys - Bhat * xs) / n)
    se_a  <- sqrt(fpc * var(ys) / n)
    e <- c(ratio = est_r, srs = est_a, Bhat = Bhat,
           lo_r = est_r - ZCRIT * se_r, hi_r = est_r + ZCRIT * se_r,
           lo_a = est_a - ZCRIT * se_a, hi_a = est_a + ZCRIT * se_a,
           cov_r = as.numeric(abs(est_r - ybarU) <= ZCRIT * se_r),
           cov_a = as.numeric(abs(est_a - ybarU) <= ZCRIT * se_a))
    rv$s    <- s
    rv$n    <- n
  ...
  output$mainplot <- renderPlot({
    ...
    req(s)
    n   <- rv$n
    cur <- rv$ests[NROW(rv$ests), ]
    Bhat   <- cur[["Bhat"]]
    est_r  <- cur[["ratio"]]
    est_a  <- cur[["srs"]]
    ci_r   <- c(cur[["lo_r"]], cur[["hi_r"]])
    ci_a   <- c(cur[["lo_a"]], cur[["hi_a"]])
    pred_r <- Bhat * pop$x[-s]
    ...
    ## ---- left: population, sample, both sets of predictions ----
    par(mar = c(4.2, 4.8, 3.0, 1.0))
    plot(NA, xlim = c(0, 1), ylim = c(0, 1), axes = FALSE,
         xlab = "x: acres87 (thousands)", ylab = "y: acres92 (thousands)",
         main = sprintf("Population, sample and predictions, n = %d", n))
    rect(0, tfy(lo), 1, tfy(hi), col = col_band, border = NA)
    points(tfx(pop$x), tfy(ypl), pch = 16, cex = 0.6, col = col_pop)

    xg <- seq(rgx[1], rgx[2], length.out = 1500)
    lines(tfx(xg), tfy(B * xg), col = col_true, lwd = 1.8)            # true y = Bx
    lines(tfx(xg), tfy(Bhat * xg), col = col_rat, lwd = 2, lty = 2)   # fitted ratio
    abline(h = tfy(est_a), col = col_avg, lwd = 2, lty = 2)           # fitted average
    if (isTRUE(input$shp)) {
      points(tfx(pop$x[-s]), tfy(rep(est_a, N - n)), pch = 16, cex = 0.6, col = col_avg)
      points(tfx(pop$x[-s]), tfy(pred_r), pch = 16, cex = 0.6, col = col_rat)
    }
    points(tfx(pop$x[s]), tfy(ypl[s]), pch = 16, cex = 1.2, col = col_samp)

    abline(h = tfy(ybarU), col = col_samp, lty = 3)
    ci_bar(0.02, est_r, ci_r, col_rat, 16, tfy)
    ci_bar(0.06, est_a, ci_a, col_avg, 17, tfy)

    mask_gap(bky, 2); mask_gap(bkx, 1)
    box(); draw_axis(bky, 2); draw_axis(bkx, 1)
    legend("topleft", bty = "n", cex = 0.78,
           pch = c(16, NA, NA, NA, NA), lty = c(NA, 1, 2, 2, 3),
           lwd = c(NA, 1.8, 2, 2, 1),
           col = c(col_samp, col_true, col_rat, col_avg, col_samp),
           legend = c("sampled y", "true Bx", "ratio fit", "average fit",
                      "population mean"))
    ...
    red_mc <- if (nrep > 5) 1 - var(rv$ests[, "ratio"]) / var(rv$ests[, "srs"]) else NA
    cov_txt <- function(col) if (nrep >= 10)
      sprintf("%.1f%%", 100 * mean(rv$ests[, col])) else "--"
    ...
    ci_bar(0.02, est_r, ci_r, col_rat, 16, cap = 0.012)
    ci_bar(0.06, est_a, ci_a, col_avg, 17, cap = 0.012)
    box()
    vy <- pretty(c(lo, hi), 5)
    vy <- vy[vy >= lo & vy <= hi]
    ...
    legend("topleft", bty = "n", cex = 0.9, title = "95% CI coverage",
           fill = c(col_rat, col_avg), border = NA,
           legend = c(paste("ratio", cov_txt("cov_r")),
                      paste("average", cov_txt("cov_a"))))
  })
}
