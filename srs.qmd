...
draw; the grey dashed bar beside it is the theoretical margin of deviation
...
server <- function(input, output, session) {
  ...
  output$mainplot <- renderPlot({
    ...
    text(1.10, cut + lane_w / 2, sprintf("compressed: > %s (%d counties)", fmt(cut), n_tail),
    ...
    if (length(rv$means) == 0) {
      plot(1, type = "n", xlim = c(0, 1), ylim = xlim_b, bty = "n", xaxt = "n",
    ...
    plot(NA, xlim = c(0, top * 1.30), ylim = xlim_b, bty = "n",
         xlab = "Density", ylab = "Sample mean of acres92",
         main = "Sampling distribution of the sample mean")
    ...
    x2        <- top * 0.74
    segments(x2, ybarU - me_normal, x2, ybarU + me_normal, col = "grey40", lwd = 2, lty = 2)
    segments(x2 - top * 0.03, ybarU + c(-1, 1) * me_normal,
              x2 + top * 0.03, ybarU + c(-1, 1) * me_normal, col = "grey40", lwd = 2)
