...
$M_i$ as the auxiliary variable, and the plain sample average of the SRS. The
cluster estimator's confidence interval uses the full two-stage variance --- the
between-cluster term plus the within-cluster term
$\sum_i \hat{v}_i / (nN\overline{M}^2)$ --- exactly as `cluster_ratio()`
computes it, on $n - 1$ degrees of freedom.
...
the cap in a single step. Changing any design setting clears the accumulated
history, so every coverage rate on screen belongs to the settings currently
shown.
