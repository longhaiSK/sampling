...
Because `na.strings = "-99"` is applied at import, every `-99` is already `NA` in `agpop`. All later filtering therefore uses `is.na()`; comparisons such as `acres92 != -99` would silently keep nothing out and, worse, propagate `NA` through the condition.
...
Printing a few thousand rows into an HTML document consumes vertical space and slows the browser. `gt` can place the full table inside a fixed-height scrolling container. A static target such as Typst or PDF has no scrollbar, so there the head of the data is shown instead.
...
agpop |>
  ...
  tab_options(
    container.height = px(400),  # restrict the table height
    container.overflow.y = TRUE  # enable the vertical scroll bar
  )
```

With `df-print: paged` set in the YAML header, a plain data frame is instead rendered as a paged table:
...
The scrolling container embeds every row in the HTML, so file size grows with the data. For very large tables, pipe through `slice_head()` first.
